                ...
                const newToken = jwt.sign({
                    ...
                    context: {
                        user: {
                            ...
                            role: decodedExpToken.context.user.role
                        }
                    }
...
export const login = async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(404).json({data: undefined, type: 'warning', message: 'User does not exist'})
        }

        //Check if the passwords match
        const correctPassword = await bcrypt.compare(password, user.password);
        if (!correctPassword) {
            return res.status(401).json({data: undefined, type: 'error', message: 'Incorrect Password'})
        }
...
export const register = async (req, res) => {
    try {
        ...
        if (user) {
            return res.status(409).json({data: undefined, type: 'warning', message: 'User already exists'})
        }

        //Check if the passwords are equal
        if (password !== confirmPassword) {
            return res.status(400).json({data: undefined, type: 'warning', message: 'Passwords must match'})
        }
        ...
        if (username.length > 15) {
            return res.status(400).json({
                data: undefined,
                type: 'warning',
                message: 'Username must be 15 or less characters'
            })
        }
