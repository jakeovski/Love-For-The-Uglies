...
export const editProfile = async(req,res) => {
    ...
    try{
        ...
        if(oldUsername !== newUserData.username) {
            ...
            if(user) return res.status(403).json({data:undefined,type:'warning',message:'Username already exists'});
        }
...
export const changePassword = async(req,res) => {
    ...
    try{
        ...
        if (!oldPassword || !newPassword || !newPasswordConfirm) return res.status(403).json({data:undefined,type:'warning',message:'All fields must be entered'});

        //Check new password length
        if (newPassword.length < 8) return res.status(403).json({data:undefined,type:'warning',message:'Password must be at least 8 characters'});

        //Check if the passwords match
        if (newPassword !== newPasswordConfirm) return res.status(403).json({data:undefined,type:'warning',message:'Passwords are not equal'});
        ...
        if (!correctPassword) {return res.status(403).json({data:undefined,type:'error',message:'Incorrect Password'})}
